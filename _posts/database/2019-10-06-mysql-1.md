...
Here, we should know what 'Primary key' is. A primary key is a field in a table which uniquely identifies each row in a database table. 
...
It's done by 'Foreign Key'. It is a field in one table that refers to the Primary key in another table.
